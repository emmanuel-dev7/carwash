...
@section('content')
    ...
    <form action="{{ route('employees.update', $employee) }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" class="border p-2 w-full" value="{{ $employee->name }}" required>
        </div>
        <div class="mb-4">
            <label for="role" class="block text-sm font-medium">{{ __('Role') }}</label>
            <input type="text" name="role" id="role" class="border p-2 w-full" value="{{ $employee->role }}" required>
        </div>
        <div class="mb-4">
            <label for="hours_worked" class="block text-sm font-medium">{{ __('Hours Worked') }}</label>
            <input type="number" name="hours_worked" id="hours_worked" class="border p-2 w-full" value="{{ $employee->hours_worked }}" step="0.01">
        </div>

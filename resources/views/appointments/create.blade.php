...
@section('content')
    <h1 class="text-2xl font-bold mb-6">{{ __('Add Appointment') }}</h1>
    <form action="{{ route('appointments.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" name="start_time" id="start_time" class="border p-2 w-full" required>
        </div>
        <div class="mb-4">
            <label for="end_time" class="block text-sm font-medium">{{ __('End Time') }}</label>
            <input type="datetime-local" name="end_time" id="end_time" class="border p-2 w-full" required>
        </div>
        <div class="mb-4">
